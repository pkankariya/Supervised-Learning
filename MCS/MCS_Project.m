...
%% Computing ezroc
[roc_pca, EER_pca, area_pca, EERthr_pca, ALLthr_pca, d_pca, gen_pca, imp_pca] = ezroc3(norm_dist_mode1,score_mode1,2,'ROC_mode1',1)
GAR_mode1 = transpose(roc_pca(1,:));
FAR_mode1 = transpose(roc_pca(2,:));
...
for i_lda = 1:(size(score_lda,1)/5)
    score_lda(r_lda:r_lda+4, c_lda:c_lda+4) = 0;
%% Computing scores using euclidean distance between the matrices
    r_lda = r_lda+5;
    c_lda = c_lda+5;
end
...
%% Computing ezroc
[roc_lda, EER_lda, area_lda, EERthr_lda, ALLthr_lda, d_lda, gen_lda, imp_lda] = ezroc3(norm_dist_lda,score_lda,2,'ROC_lda',1)
GAR_lda = transpose(roc_lda(1,:));
FAR_lda = transpose(roc_lda(2,:));

%% Mode A - score level fusion using the score matrices identified using PCA and LDA

% Maximum of score matrices enabling score level fusion
score_max_mcs = max(norm_dist_mode1, norm_dist_lda);

% Computing ROC and accuracy of classifier using max score level fusion
[roc_max, EER_max, area_max, EERthr_max, ALLthr_max, d_max, gen_max, imp_max] = ezroc3(score_max_mcs,score_lda,2,'ROC_max_score',1)
GAR_max_score = transpose(roc_max(1,:));
FAR_max_score = transpose(roc_max(2,:));

% Minimum of score matrrices enabling score level fusion
score_min_mcs = min(norm_dist_mode1, norm_dist_lda);

% Computing ROC and accuracy of classifier using min score level fusion
[roc_min, EER_min, area_min, EERthr_min, ALLthr_min, d_min, gen_min, imp_min] = ezroc3(score_min_mcs,score_lda,2,'ROC_min_score',1)
GAR_min_score = transpose(roc_min(1,:));
FAR_min_score = transpose(roc_min(2,:));

% Minimum of score matrrices enabling score level fusion
score_avg_mcs = (norm_dist_mode1+norm_dist_lda)/2;

% Computing ROC and accuracy of classifier using avg score level fusion
[roc_avg, EER_avg, area_avg, EERthr_avg, ALLthr_avg, d_avg, gen_avg, imp_avg] = ezroc3(score_avg_mcs,score_lda,2,'ROC_avg_score',1)
GAR_avg_score = transpose(roc_avg(1,:));
FAR_avg_score = transpose(roc_avg(2,:));

%% Combining multiple ROC curves into a single graph

% Title of the comparision graph along with labels for x and y axes
title('ROC'), xlabel('FAR'), ylabel('GAR');

% Plotting all 5 ROC curves onto a single graph
hold on; 
plot(FAR_mode1, GAR_mode1, 'linewidth', 3);
plot(FAR_lda, GAR_lda, 'linewidth', 3);
plot(FAR_max_score, GAR_max_score, 'linewidth', 3);
plot(FAR_min_score, GAR_min_score, 'linewidth', 3);
plot(FAR_avg_score, GAR_avg_score, 'linewidth', 3); 

% Title of each of the ROC curve displayed
legend('PCA', 'LDA', 'Max_Score_Fusion', 'Min_Score_Fusion', 'Avg_Score_Fusion');
hold off;

%% Decision level fusion for performance evaluation of PCA vs LDA

% Decision level fusion @ 5% threshold of FAR

% Evaluating PCA distances against 5% threshold associated to FAR
decision_value_pca_5 = ((-0.7815)+(-0.7631))/2;
result_pca_5 = norm_dist_mode1 > decision_value_pca_5;

% Evaluating LDA distances against 5% threshold assocaited to FAR
decision_value_lda_5 = (0.1466+0.1599+0.1732+0.1865+0.1997)/5;
result_lda_5 = norm_dist_lda > decision_value_lda_5;

% Evaluating scores based on decision level analysis against 5% threshold
% associated to FAR
final_decision_5 = zeros(size(result_pca_5))
for i=1:size(result_pca_5,1)
    for j=1:size(result_pca_5,2)
        if(result_pca_5(i,j)==result_lda_5(i,j))
            final_decision_5(i,j) = result_lda_5(i,j);
        else 
% %             final_decision(i,j) = randi([0,1]);
            final_decision_5(i,j) = 1;
        end
    end
end

% Comparing scores identified using decision-level fusion with the original
% score matrix @ 5% threshold associated to FAR
decision_5 = repmat("Y",size(final_decision_5));
count_yes_5 = 0;
count_no_5 = 0;
for i=1:size(final_decision_5,1)
    for j=1:size(final_decision_5,2)
        if(final_decision_5(i,j)==score_mode1(i,j))
            decision_5(i,j) = "Y";
            count_yes_5 = count_yes_5 + 1;
        else 
% %             final_decision(i,j) = randi([0,1]);
            decision_5(i,j) = "N";
            count_no_5 = count_no_5 + 1;
        end
    end
end

fprintf('Matched Score @ 5% FAR threshold: %d\n', count_yes_5);
fprintf('Unmatched score @ 5% FAR threshold: %d\n', count_no_5);

% Genuine scores mislabelled as imposter and Imposter scores mislabelled as
% genuine
mislabel_genuine_5 = 0;
mislabel_imposter_5 = 0;
for i=1:size(final_decision_5,1)
    for j=1:size(final_decision_5,2)
        if (score_mode1(i,j) == 0 && final_decision_5(i,j)==1)
            mislabel_genuine_5 = mislabel_genuine_5 + 1;
        elseif (score_mode1(i,j) == 1 && final_decision_5(i,j)==0)
                mislabel_imposter_5 = mislabel_imposter_5 + 1;
        end
    end
end

% Decision level fusion @ 10% threshold of FAR

% Evaluating PCA distances against 10% threshold associated to FAR
decision_value_pca_10 = ((-1.0760)+(-1.0576))/2;
result_pca_10 = norm_dist_mode1 > decision_value_pca_10;

% Evaluating LDA distances against 10% threshold assocaited to FAR
decision_value_lda_10 = ((-0.1855)+(-0.1722)+(-0.1589)+(-0.1456)+(-0.1323))/5;
result_lda_10 = norm_dist_lda > decision_value_lda_10;

% Evaluating scores based on decision level analysis against 10% threshold
% associated to FAR
final_decision_10 = zeros(size(result_pca_10))
for i=1:size(result_pca_10,1)
    for j=1:size(result_pca_10,2)
        if(result_pca_10(i,j)==result_lda_10(i,j))
            final_decision_10(i,j) = result_lda_10(i,j);
        else 
% %             final_decision_10(i,j) = randi([0,1]);
            final_decision_10(i,j) = 1;
        end
    end
end

% Comparing scores identified using decision-level fusion with the original
% score matrix @ 10% threshold associated to FAR
decision_10 = repmat("Y",size(final_decision_10));
count_yes_10 = 0;
count_no_10 = 0;
for i=1:size(final_decision_10,1)
    for j=1:size(final_decision_10,2)
        if(final_decision_10(i,j)==score_mode1(i,j))
            decision_10(i,j) = "Y";
            count_yes_10 = count_yes_10 + 1;
        else 
% %             final_decision_10(i,j) = randi([0,1]);
            decision_10(i,j) = "N";
            count_no_10 = count_no_10 + 1;
        end
    end
end

fprintf('Matched Score @ 10% FAR threshold: %d\n', count_yes_10);
fprintf('Unmatched score @ 10% FAR threshold: %d\n', count_no_10);

% Genuine scores mislabelled as imposter and Imposter scores mislabelled as
% genuine
mislabel_genuine_10 = 0;
mislabel_imposter_10 = 0;
for i=1:size(final_decision_10,1)
    for j=1:size(final_decision_10,2)
        if (score_mode1(i,j) == 0 && final_decision_10(i,j)==1)
            mislabel_genuine_10 = mislabel_genuine_10 + 1;
        elseif (score_mode1(i,j) == 1 && final_decision_10(i,j)==0)
                mislabel_imposter_10 = mislabel_imposter_10 + 1;
        end
    end
end
